...
function dtr_register_workbooks_webinar($data) {
    ...
    $add_questions = isset($data['add_questions']) ? sanitize_textarea_field($data['add_questions']) : '';
    ...
    try {
        ...
        // Build ticket payload with speaker question(s) and sponsor opt-in data
        $ticket_payload = [[
            'event_id' => (int)$workbook_reference,
            'person_id' => $person_id,
            'name' => $name,
        ...
        // Combine speaker_question and add_questions if both are present
        $combined_questions = '';
        if (!empty($speaker_question) && !empty($add_questions)) {
            $combined_questions = $speaker_question . "\n" . $add_questions;
        } elseif (!empty($speaker_question)) {
            $combined_questions = $speaker_question;
        } elseif (!empty($add_questions)) {
            $combined_questions = $add_questions;
        }
        if (!empty($combined_questions)) {
            $ticket_payload[0]['cf_event_ticket_speaker_questions'] = $combined_questions;
            dtr_webinar_debug_log('INFO', 'Adding combined speaker/add_questions to ticket', ['combined_questions' => $combined_questions]);
        }

        if ($sponsor_optin === true || $sponsor_optin === 'true' || $sponsor_optin === 1) {
            $ticket_payload[0]['cf_event_ticket_sponsor_optin'] = true;
            dtr_webinar_debug_log('INFO', 'Adding sponsor opt-in to ticket', ['sponsor_opt_in' => 'Yes']);
        } else {
            $ticket_payload[0]['cf_event_ticket_sponsor_optin'] = false;
            dtr_webinar_debug_log('INFO', 'Adding sponsor opt-in to ticket', ['sponsor_opt_in' => 'No']);
        }

        dtr_webinar_debug_log('INFO', 'Complete ticket payload created with speaker/add_questions and opt-in data', $ticket_payload);
        ...
        if ($final_ticket_id) {
            ...
            dtr_webinar_debug_log('ERROR', 'No ticket ID returned', $response);
            return ['error' => 'Failed to get ticket ID from response'];
        }

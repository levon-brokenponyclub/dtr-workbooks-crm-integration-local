...
function dtr_ninja_forms_lead_generation_handler($form_data) {
    try {
        ...
        // DEBUG: Log all field keys and values in fields
        if (isset($form_data['fields']) && is_array($form_data['fields'])) {
            $all_keys = [];
            foreach ($form_data['fields'] as $field) {
                if (isset($field['key'])) {
                    $k = $field['key'];
                    $v = isset($field['value']) ? $field['value'] : '[NO VALUE]';
                    dtr_lead_debug("[DEBUG] fields[] key: $k | value: " . print_r($v, true));
                    $all_keys[] = $k;
                }
            }
            dtr_lead_debug('[DEBUG] All field keys in fields[]: ' . implode(', ', $all_keys));
        }

        // DEBUG: Log all field keys and values in fields_by_key
        if (isset($form_data['fields_by_key']) && is_array($form_data['fields_by_key'])) {
            $all_keys = [];
            foreach ($form_data['fields_by_key'] as $k => $field) {
                $v = isset($field['value']) ? $field['value'] : '[NO VALUE]';
                dtr_lead_debug("[DEBUG] fields_by_key key: $k | value: " . print_r($v, true));
                $all_keys[] = $k;
            }
            dtr_lead_debug('[DEBUG] All field keys in fields_by_key: ' . implode(', ', $all_keys));
        }
...
function dtr_handle_webinar_form_submission($form_data) {
    ...
    // Try to extract from fields_by_key if present (newer Ninja Forms structure)
    if (isset($form_data['fields_by_key']) && is_array($form_data['fields_by_key'])) {
        foreach ($form_data['fields_by_key'] as $key => $field_data) {
            ...
            if (strpos($key, 'acf_question_') === 0) {
                $acf_questions[$key] = $value;
                dtr_lead_debug("ACF Question $key: $value");
            }
            switch ($key) {
                case 'webinar_title':
                    $webinar_title = $value;
    ...
    // Aggregate all ACF questions into a single string for add_questions
    if (!empty($acf_questions)) {
        $add_questions = "";
        foreach ($acf_questions as $qkey => $qval) {
            $add_questions .= "$qkey: $qval\n";
        }
        $add_questions = trim($add_questions);
        dtr_lead_debug("Aggregated add_questions: $add_questions");
    }
    
    // Log the add_questions value for debugging
    dtr_lead_debug("add_questions field: " . ($add_questions ?: 'EMPTY'));
    ...
    // Pass add_questions to the registration function
    dtr_call_webinar_registration($post_id, $participant_email, $speaker_question, $sponsor_optin, $add_questions);
}

/**
 * Call the core webinar registration function
 */
function dtr_call_webinar_registration($post_id, $participant_email, $speaker_question, $sponsor_optin, $add_questions = '') {
    ...
    $registration_data = array(
        ...
        'add_questions' => $add_questions // <-- Pass through
    );
    ...
    dtr_lead_debug("  - add_questions: " . $registration_data['add_questions']);
